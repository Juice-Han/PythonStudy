...
if options.solve == False:
    ...
    hits = 0
    miss = 0
    access = [0] * cachesize # cachesize와 같은 길이의 access bit 저장하는 배열
    ...
    for nStr in addrList:
        ...
        try:
            ...
            if policy == 'FIFO' and  access[idx] == 0:
                access[idx] = 1 # policy가 FIFO이고, access bit의 값이 0이라면 1로 값 갱신
        ...
        if idx == -1:
            ...
            if count == cachesize:
                ...
                if policy == 'LRU':
                    victim = memory.pop(0)
                elif policy == 'FIFO': # FIFO policy 조건 분기
                    index = 0
                    for i in range(len(memory)):
                        if access[i] == 1:
                            access[i] = 0 # 만약 access bit의 값이 1이라면 0으로 재설정 후 다시 탐색
                            index += 1
                            continue
                        victim = memory.pop(i) # 탐색 중 access bit의 값이 0인 page를 victim으로 설정
                        access.pop(i) # 메모리 조정에 맞춰 access bit 순서 변경
                        access.append(0)
